  ...
  template: `
    <div class="page-hero">
      ...
      <div class="page-hero__top">
        <span class="page-hero__no">№ 03 — PUBLICATIONS</span>
        <span class="page-hero__rule"></span>
        <span class="page-hero__lang">JOURNAL · ISSN</span>
      </div>
      <h1 class="page-hero__title">出<span class="acc">版</span></h1>
      <p class="page-hero__sub">
        <span class="page-hero__caption">Édition —</span>
        期刊论文、合集与其他正式出版物
      </p>
    </div>

    <div class="constr-sect-label">
      <span class="constr-sect-label__no">№ 03·1</span>
      <span class="constr-sect-label__rule constr-sect-label__rule--thick"></span>
      <span class="constr-sect-label__title">著录</span>
      <span class="constr-sect-label__rule"></span>
      <span class="constr-sect-label__meta" *ngIf="pubs.length">{{ pubs.length }} 项</span>
    </div>

    <ul class="article-list indexed-list" *ngIf="pubs.length">
      <li class="indexed-row" *ngFor="let p of pubs; let i = index">
        <span class="list-index">{{ pad(i + 1) }}</span>
        <div class="indexed-row__body">
          <app-article-card [article]="p" linkPrefix="/publication"></app-article-card>
        </div>
      </li>
    </ul>

    <p *ngIf="!pubs.length" class="empty">尚无出版物</p>
  `,
  styles: [`
    .page-hero {
      position: relative;
...
export class PublicationListComponent implements OnInit {
  ...
  ngOnInit() {
    this.api.getPublications().subscribe(p => this.pubs = p);
  }
}
